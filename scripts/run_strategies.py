...
def run_option(name: str, spec: dict, book: dict, df: pd.DataFrame,
    ...
    option_fee = pcfg.get("option_fee_per_contract",
                          paper.DEFAULT_OPTION_FEE_PER_CONTRACT)
    pos = book["position"]
    if pos:
        ...
        if contract is None:
            ...
            filled = paper.open_option(book, contract, pcfg["option_alloc"], today,
                                       fee_per_contract=option_fee)
            if filled is None:
                one_contract_cost = contract["fill"] * 100 + option_fee
                detail = (f"entry signal but 1 contract "
                          f"(~${one_contract_cost:.0f}) exceeds book cash")
            else:
